...
DATA = json.loads((ROOT / "_rawdata" / "gym.json").read_text(encoding="utf-8"))
...
for region in sorted(counts):
    ...
    page_title = f"{full} 헬스장"
    title_h1 = f"{full} 헬스장 {cnt}곳"
    subtitle = f"{full} 지역 헬스장을 검색하세요."

    content = f"""---
layout: region
title: {page_title}
description: {full} 헬스장 정보. 위치, 전화번호를 확인하세요.
do_name: {region}
title_h1: {title_h1}
subtitle: {subtitle}
---
